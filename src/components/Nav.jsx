...
export default function Nav() {
  ...
  return (
    <ul className="max-w-full overflow-auto grid grid-flow-col auto-cols-[1fr] gap-2 lg:gap-0 items-center">
        {navMain.map((e) => (
            <li className="hover:bg-gray-800/20 rounded-md hover:shadow-sm border-[1px] border-transparent hover:border-gray-800/30 grid place-items-center" key={e.titulo}>
                <NavLink
                to={e.link}
                className={({isActive}) => {
                    return (
                        ...
                        (!isActive
                        ? 'text-gray-400'
                        : 'border-b-sky-500 text-gray-200')
                    )
                }}>
                    {e.titulo}
                </NavLink>
